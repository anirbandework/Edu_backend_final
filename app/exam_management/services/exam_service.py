...
class ExamService:
    ...
    async def get_student_report_card(self, tenant_id: UUID, student_id: UUID,
                                      academic_year: Optional[str] = None) -> Dict[str, Any]:
        """Build a report card by aggregating the student's actual exam marks
        (StudentExamMark) grouped by subject. Uses populated exam data rather
        than the separate (unused) student_grades table."""
        from sqlalchemy import select
        ...
        stmt = select(
            ...
            and_(
                Exam.tenant_id == tenant_id,
                Exam.is_deleted == False,
                StudentExamMark.obtained_marks.isnot(None),
            )
        )
        if academic_year:
            stmt = stmt.filter(Exam.academic_year == academic_year)
        stmt = stmt.order_by(Exam.subject, Exam.exam_date)
        ...
        for r in rows:
            ...
            s = subjects.setdefault(subj, {"subject": subj, "exams": [],
                                           "_pct_sum": 0.0, "_count": 0})
            pct = float(r.percentage) if r.percentage is not None else (
                (float(r.obtained_marks) / float(r.total_marks) * 100.0)
                if r.total_marks else 0.0)
            s["exams"].append({
                "exam_name": r.exam_name,
                "exam_type": r.exam_type,
                "obtained_marks": r.obtained_marks,
        ...
        for s in subjects.values():
            avg = s["_pct_sum"] / s["_count"] if s["_count"] else 0.0
            passed = avg >= 40.0
            if passed:
                subjects_passed += 1
            overall_sum += s["_pct_sum"]
            overall_count += s["_count"]
            subject_list.append({
                "subject": s["subject"],
                "exams": s["exams"],
                "average_percentage": round(avg, 2),
                "grade": self._letter_grade(avg),
                "passed": passed,
            })

        overall_pct = (overall_sum / overall_count) if overall_count else 0.0
        ...
        return {
            ...
            "total_exams": overall_count,
            "overall_percentage": round(overall_pct, 2),
            "overall_grade": self._letter_grade(overall_pct),
            "result": "PASS" if total_subjects > 0 and subjects_passed == total_subjects else
                      ("FAIL" if total_subjects > 0 else "N/A"),
