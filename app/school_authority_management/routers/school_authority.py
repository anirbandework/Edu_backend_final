...
from ...school_authority_management.services.school_authority_service import SchoolAuthorityService
...
async def create_authority(
    ...
    # Override client-supplied tenant_id: non-super-admins may only create within
    # their own tenant; super-admins may target the supplied tenant.
    if not principal.is_super_admin:
        authority_dict["tenant_id"] = principal.tenant_id
...
async def get_authority(
    ...
    authority = await service.get(authority_id, tenant_id=scope_tenant)
...
async def update_authority(
    ...
    authority = await service.update(authority_id, update_dict, tenant_id=scope_tenant)
...
async def delete_authority(
    ...
    service = SchoolAuthorityService(db)
    scope_tenant = None if principal.is_super_admin else principal.tenant_id
    success = await service.soft_delete(authority_id, tenant_id=scope_tenant)
...
async def get_authorities_by_tenant(
    ...
    # Non-super-admins may only query their own tenant (super-admin bypass inside).
...
async def bulk_update_permissions(
    ...
    effective_tenant = permission_data.tenant_id if principal.is_super_admin else principal.tenant_id
...
async def bulk_update_positions(
    ...
    effective_tenant = position_data.tenant_id if principal.is_super_admin else principal.tenant_id
...
async def get_authority_statistics(
    ...
    # Non-super-admins may only view stats for their own tenant.
    assert_same_tenant(principal, tenant_id)
    service = SchoolAuthorityService(db)
