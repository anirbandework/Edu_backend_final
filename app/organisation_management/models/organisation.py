# app/models/shared/organisation.py
"""Organisation (Organisation) model definition."""
...
class Organisation(Base):
    ...
    annual_tuition = Column(Numeric(10, 2), nullable=True)
    registration_fee = Column(Numeric(8, 2), nullable=True)
    ...
    org_type = Column(String(20), default="School")
    levels_offered = Column(ARRAY(String), nullable=True)
    established_year = Column(Integer)
    accreditation = Column(String(50))
    language_of_instruction = Column(String(20), default="English")

    # The institution group this organisation belongs to. Visibility/management is
    # by group (every admin of the group sees all the group's organisations); each
    # organisation's data stays isolated. FK + index live in migrations.
    group_id = Column(UUID(as_uuid=True), nullable=True, index=True)

    # The admin (authority) who CREATED this organisation (audit only — management
    # is group-scoped, not per-creator). NULL for super-admin-created ones.
    ...
    authorities = relationship("Authority", back_populates="organisation", cascade="all, delete-orphan", lazy="dynamic")
    ...
    __table_args__ = (
        # Unique constraints
        UniqueConstraint('email', name='uq_organisation_email'),
        UniqueConstraint('phone', name='uq_organisation_phone'),
        UniqueConstraint('name', 'address', name='uq_organisation_name_address'),
        UniqueConstraint('code', name='uq_organisation_code'),
        ...
        Index('idx_org_type_active', 'org_type', 'is_active'),
