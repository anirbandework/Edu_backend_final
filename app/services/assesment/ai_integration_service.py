...
from ...schemas.assesment.quiz_validation_schemas import QuestionType, DifficultyLevel
...
class AIService:
    def __init__(self):
        ...
        print(f"AI Service initialized - API Key: {'Found' if self.api_key else 'Missing'}")
    ...
    async def _make_request(self, messages: List[Dict], model: str = "sonar") -> str:
        ...
        try:
            print(f"Making API request to Perplexity with model: {model}")
            ...
            print(f"API Success - Content length: {len(content)}")
            return content
