...
from ...assessment_management.schemas.quiz_validation_schemas import QuizAttemptResponse
...
def _eff_owner(principal: Principal, owner_id: UUID) -> UUID:
    """Owner id for staff-owned resources: a teacher is forced to themselves; an
    authority/super-admin may act on any teacher in their tenant."""
    if principal.is_super_admin or principal.is_authority:
        return owner_id
    return UUID(str(principal.user_id))


def _eff_subject(principal: Principal, subject_id: UUID) -> UUID:
    """Subject (student) id: staff/super-admin may target any student in-tenant; a
    student is forced to their own id (no cross-user access)."""
    if principal.is_super_admin or principal.is_staff:
        return subject_id
    return UUID(str(principal.user_id))


# AI Quiz Creation and Management (staff)
...
async def create_ai_quiz(
    ...
    try:
        ...
        return await ai_quiz_service.create_complete_ai_quiz(
            db=db, request=request, teacher_id=_eff_owner(principal, teacher_id), tenant_id=_eff_tenant(principal, tenant_id))
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to create AI quiz: {str(e)}")
...
async def get_ai_quiz_templates(
    ...
    try:
        ...
        return await ai_quiz_service.get_quiz_templates(
            db=db, subject=subject, grade_level=grade_level, tenant_id=_eff_tenant(principal, tenant_id))
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get templates: {str(e)}")
...
async def generate_quiz_from_template(
    ...
    try:
        ...
        return await ai_quiz_service.generate_from_template(
            db=db, template_request=template_request, teacher_id=_eff_owner(principal, teacher_id),
            tenant_id=_eff_tenant(principal, tenant_id), class_ids=class_ids)
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to generate from template: {str(e)}")


# Student AI Quiz Interaction (student self-service)
...
async def get_ai_quiz_for_student(
    ...
    try:
        ...
        return await ai_quiz_service.get_ai_quiz_for_student(
            db=db, quiz_id=quiz_id, student_id=_eff_subject(principal, student_id), tenant_id=_eff_tenant(principal, tenant_id))
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Quiz not found: {str(e)}")
...
async def start_ai_quiz_attempt(
    ...
    try:
        ...
        return await ai_quiz_service.start_ai_quiz_attempt(
            db=db, request=request, student_id=_eff_subject(principal, student_id), tenant_id=_eff_tenant(principal, tenant_id))
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to start attempt: {str(e)}")
...
async def get_ai_hint(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get hint: {str(e)}")
...
async def get_student_available_ai_quizzes(
    ...
    try:
        ...
        return await ai_quiz_service.get_student_available_ai_quizzes(
            db=db, student_id=_eff_subject(principal, student_id), tenant_id=_eff_tenant(principal, tenant_id))
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get available quizzes: {str(e)}")
...
async def get_ai_quiz_results(
    ...
    try:
        ...
        return await ai_quiz_service.get_ai_quiz_results(
            db=db, attempt_id=attempt_id, student_id=_eff_subject(principal, student_id), tenant_id=_eff_tenant(principal, tenant_id))
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Results not found: {str(e)}")
...
async def get_student_ai_quiz_history(
    ...
    try:
        ...
        return await ai_quiz_service.get_student_ai_quiz_history(
            db=db, student_id=_eff_subject(principal, student_id), tenant_id=_eff_tenant(principal, tenant_id), subject=subject)
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get history: {str(e)}")


# Teacher AI Quiz Management (staff)
...
async def get_teacher_ai_quiz_dashboard(
    ...
    try:
        ...
        return await ai_quiz_service.get_teacher_ai_dashboard(
            db=db, teacher_id=_eff_owner(principal, teacher_id), tenant_id=_eff_tenant(principal, tenant_id))
    except HTTPException:
        raise
    except Exception as e:
...
async def get_teacher_ai_quizzes(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get teacher quizzes: {str(e)}")
...
async def get_ai_quiz_class_analytics(
    ...
    try:
        ...
        return await ai_quiz_service.get_ai_class_analytics(
            db=db, quiz_id=quiz_id, class_id=class_id, teacher_id=_eff_owner(principal, teacher_id), tenant_id=_eff_tenant(principal, tenant_id))
    except HTTPException:
        raise
    except Exception as e:
...
async def delete_ai_quiz(
    ...
    try:
        ...
        success = await ai_quiz_service.delete_ai_quiz(
            db=db, quiz_id=quiz_id, teacher_id=_eff_owner(principal, teacher_id), tenant_id=_eff_tenant(principal, tenant_id))
        if not success:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Quiz not found or unauthorized")
        return {"message": "AI quiz deleted successfully"}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to delete quiz: {str(e)}")
...
async def update_ai_quiz_status(
    ...
    try:
        ...
        success = await ai_quiz_service.update_ai_quiz_status(
            db=db, quiz_id=quiz_id, is_active=is_active, teacher_id=_eff_owner(principal, teacher_id), tenant_id=_eff_tenant(principal, tenant_id))
        if not success:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Quiz not found or unauthorized")
