...
class AuthorityService(BaseService[Authority]):
    def __init__(self, db: AsyncSession):
        super().__init__(Authority, db)
    
    async def get_by_organisation(self, organisation_id: UUID) -> List[Authority]:
        """Get all authorities for a specific organisation/organisation"""
        stmt = select(self.model).where(
            self.model.organisation_id == organisation_id,
    ...
    async def get_by_authority_id(self, authority_id: str) -> Optional[Authority]:
    ...
    async def get_active_authorities(self, organisation_id: Optional[UUID] = None) -> List[Authority]:
        """Get all active authorities, optionally filtered by organisation"""
        stmt = select(self.model).where(
    ...
    async def create(self, obj_in: dict) -> Authority:
    ...
    async def update_login_time(self, authority_id: UUID) -> Optional[Authority]:
    ...
    async def bulk_import_authorities(self, authorities_data: List[dict], organisation_id: UUID) -> dict:
        """Bulk import authorities with duplicate detection"""
        try:
            ...
            existing_check_sql = text("""
                SELECT email, authority_id FROM authorities 
            ...
            for idx, authority_data in enumerate(authorities_data):
                try:
                    ...
                    authority_record = {
                        ...
                        "org_overview": json.dumps(authority_data.get("org_overview")) if authority_data.get("org_overview") else None,
            ...
            if insert_data:
                bulk_insert_sql = text("""
                    INSERT INTO authorities (
                        id, organisation_id, authority_id, first_name, last_name, email, phone,
                        date_of_birth, address, role, status, position, qualification,
                        experience_years, joining_date, authority_details, permissions,
                        org_overview, contact_info, last_login, created_at, updated_at, is_deleted
                    ) VALUES (
                        :id, :organisation_id, :authority_id, :first_name, :last_name, :email, :phone,
                        :date_of_birth, :address, :role, :status, :position, :qualification,
                        :experience_years, :joining_date, :authority_details, :permissions,
    ...
    async def bulk_update_status(self, authority_ids: List[str], new_status: str, organisation_id: UUID) -> dict:
    ...
    async def bulk_update_permissions(self, permission_updates: List[dict], organisation_id: UUID) -> dict:
        ...
        try:
            ...
            result = await self.db.execute(
                authorities_sql, 
                {"organisation_id": organisation_id, "authority_ids": authority_ids}
            )
    ...
    async def bulk_update_positions(self, position_updates: List[dict], organisation_id: UUID) -> dict:
    ...
    async def bulk_soft_delete(self, authority_ids: List[str], organisation_id: UUID) -> dict:
    ...
    async def get_authority_statistics(self, organisation_id: UUID) -> dict:
        ...
        try:
            ...
            result = await self.db.execute(stats_sql, {"organisation_id": organisation_id})
            ...
            position_result = await self.db.execute(position_distribution_sql, {"organisation_id": organisation_id})
